...
async def run_sprint(sprint: Sprint) -> bool:
    ...
    # ── Copy files-to-modify into sprint dir root ──
    #    These are files the agent will read and edit in-place.
    #    Source: either a prior sprint's output or the reference codebase.
    for mod_file in sprint.files_to_modify:
        dst = sprint_dir / mod_file
        if dst.exists():
            continue  # already present (e.g. from a prior sprint copy)

        # Try the reference codebase first
...
async def run_campaign(
    ...
    if dry_run:
        ...
        if not REFERENCE_DIR.exists():
            ...
            for subdir in ["codebase", "stubs", "docs"]:

...
- The .mat persistence pattern (generate → save → workspace_wipe → load)
  is a proven pattern already used by sim_config.mat, sim_state.mat,
  experiment_params.mat, and campaign_state.mat.
...
VARIABLE INFLUENT / TRANCHE SYSTEM (v4):
- DYNINFLUENT_ASM3.mat contains a long influent timeseries.  The workspace
  variable inside the file is named ASM3_Influent (Mx21 matrix).
- Column structure is identical to DRYINFLUENT: col 1 = time (fractional
  days, increments by 1/96), cols 2–21 = state variables.
- dynamicInfluent_writer.m slices non-overlapping tranches of N rows from
  ASM3_Influent, renormalizes the time column to start at 0, and saves the
  result as variable DRYINFLUENT into dryinfluent.mat (overwriting).
- benchmarkinit loads whatever is in dryinfluent.mat — the tranche must be
  written BEFORE main_sim is called.
- run_campaign.m manages the outer tranche loop:
  (a) Calls dynamicInfluent_writer to write the tranche.
  (b) Deletes workspace_steady_state_initial.mat to force SS re-run
      (each tranche has different influent composition → different baseline).
  (c) Deletes sim_state.mat to reset main_sim's iter to 1.
  (d) Runs main_sim (full experiment matrix per tranche).
  (e) Accumulates results with InfluentCondition and InfluentSourceStartDay.
- sim_days is coupled to tranche_len_days (one parameter controls both).
  Passed from run_campaign → campaign_params.mat → main_sim.
- Tranche count = floor((total_rows - start_row + 1) / tranche_len_rows).
  Partial tranches at end of source file are silently dropped.
- campaign_state.mat now also persists tranche_idx and outer-loop state
  to enable resume after crashes in the tranche loop.

FILES YOU MUST NOT MODIFY (unchanged from prior phases):
- ssASM3_influent_sampler.m  (preprocessing — generates Influent_Percentiles.mat)
- ssInfluent_writer.m  (percentile-based constant influent — separate use case)
"""

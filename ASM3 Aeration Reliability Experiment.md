...
## ASM3 Initial Influent Conditions
...
## Goals
Using the instructions of `ASM3 Aeration Reliablility Experiment` as a template, please generate a plan to extend the plan of the experiment to create a database for model training purposes. The following is a general set of requirements to fulfill to ensure that enough data is produced from the model to cover the entire space of model outputs and inputs.
### Requirements
	...
	- Effluent Damage Indicies
		...
		* NH4_dmg = (NH4_exp - NH4_nom)/NH4_limit 
			** Explanation: The distance of the aeration experiment effluent NH4 from the nominal effluent NH4. Weighted by the NH4 limit. 
				*** Source: `perf_plant_LT_DR.m` starting on line 28
	- Effluent Risk Indicies
		* Define MTBE for Energy Reduction (`MTBF_red`).
			** 24 Hour Energy Use is 6750 KWh
			** Calculate Percentage of total energy use and daily energy use during aeration reduction experiments.
				*** Consider Each Energy Data Point Calculation as 1/96th of a day (15 minute timesteps).
			** Return Period of Energy Reduction:
				***   24 Hours: 100 Years
				***   12 Hours: 50 Years
				***    6 Hours: 45 Years
				***    1 Hours: 25 Years
				***  0.5 Hours: 10 Years
				***  0.25 Hours: 2 Years
		* Define Effluent Risk Indicator:
			** (Effluent Dmg Index)/MTBE_red
## Database Requirements
### Goals
Create a database of representative samples of the model operational parameters during the experiment.
#### Database Size Requirements
A representative sample of the model performance should be the database size. This would be each aeration profile paired with each set of (Q_percentile, Influent_Vector). 

## Surrogate Model Requirements
### Goals
A learning algorithm that can be sampled by the optimizer for Model Parameters and Model Calculated Values.

## Optimizer Requirements
### Goals 
Calculate Optimial Intersection of the Risk + Cost Curves 